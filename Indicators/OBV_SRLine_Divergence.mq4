...
//mql5 int OBVm1_handle;
//mql5 int OBVm15_handle;
//int ZigZag_handle;

#define OBV_BUFFER_SIZE  60 * 12 / 15

//mql5 double obvValues[OBV_BUFFER_SIZE];
//mql5 int obvCount = 0;
...
//--- Create OBV Indicator.
   //mql5 OBVm1_handle = iOBV(NULL, PERIOD_M1, PRICE_CLOSE, 0);
   //mql5 OBVm15_handle = iOBV(NULL, PERIOD_M15, PRICE_CLOSE, 0);
...
//--- destroy timer
   ...
   //mql5 IndicatorRelease(OBVm1_handle);
   //mql5 IndicatorRelease(OBVm15_handle);
   //mql5 IndicatorRelease(ZigZag_handle);
  }
//+------------------------------------------------------------------+
...
//---
   ...
   for(double d = dL + MaxR + 0.1; d <= dH - MaxR - 0.1; d += 0.1)
     {
      int di = (d - dLNum) * 10;
      int MaxIndex = ArrayMaximum(CrossBarsNum, 2 * l, di - l);
      int MinIndex = ArrayMinimum(CrossBarsNum, 2 * l, di - l);
...
//--- Back track to find point of comparison.
   ...
   if(pins >= 2)
     {
      //mql5 obvCount = CopyBuffer(OBVm15_handle, 0, 0, OBV_BUFFER_SIZE, obvValues);
      //mql5 if(obvCount <= 0)
      //mql5    PrintFormat("Copied OBV count = %d", obvCount);
      //mql5 else
